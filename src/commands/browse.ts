...
export function registerBrowseListingsCommand(bot: Telegraf<BotContext>, prisma: PrismaClient) {
  ...
  bot.action(/browse_next_(\d+)/, async (ctx) => {
    try {
      ...
      const buttons = [];
      if (nextPage > 0) buttons.push(Markup.button.callback('⬅️ Previous', `browse_prev_${nextPage}`));
      if (nextPage < listings.length - 1) buttons.push(Markup.button.callback('Next ➡️', `browse_next_${nextPage}`));
      ...
      if (ctx.session && (ctx.session as any).navigationMessageId) {
        try {
          await ctx.telegram.editMessageText(
            ctx.chat!.id,
            (ctx.session as any).navigationMessageId,
            undefined,
            'Navigation:',
  ...
  bot.action(/browse_prev_(\d+)/, async (ctx) => {
    try {
      ...
      // Update the navigation message with new buttons
      const buttons = [];
      if (prevPage > 0) buttons.push(Markup.button.callback('⬅️ Previous', `browse_prev_${prevPage}`));
      if (prevPage < listings.length - 1) buttons.push(Markup.button.callback('Next ➡️', `browse_next_${prevPage}`));
